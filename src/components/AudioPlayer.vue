<template>
    <div class="w-full h-full flex flex-col relative" @click="handleContainerClick">
        <audio ref="audio" preload="auto" />

        <!-- 进度条容器 -->
        <div class="w-full h-3 bg-gray-200 rounded-full overflow-hidden cursor-pointer relative shadow-inner">
            <!-- 已播放进度 -->
            <div
                class="h-full bg-gradient-to-r from-stone-400 to-slate-600 transition-all duration-100 ease-out"
                :style="'width: ' + (totalTime > 0 ? (playTime / totalTime * 100) : 0) + '%;'">
            </div>
            <!-- 进度指示器 -->
            <div
                class="absolute top-0 h-full w-1 bg-white shadow-lg transform -translate-x-1/2 transition-all duration-100 ease-out"
                :style="'left: ' + (totalTime > 0 ? (playTime / totalTime * 100) : 0) + '%;'">
            </div>
        </div>

        <!-- 时间显示 -->
        <div class="flex justify-between items-center mt-1 px-1">
            <div class="text-xs text-gray-600 font-medium tabular-nums">
                {{ formatTime(playTime) }}
            </div>
            <div class="text-xs text-gray-400 font-medium tabular-nums">
        ...
        <button
            ...
            class="bg-blue-500 w-14 h-14 text-white rounded-full shadow-lg hover:drop-shadow-xl transition-all duration-300">
...
export default {
    ...
    methods: {
        ...
        handleContainerClick(event) {
            ...
            // 通知父组件更新Canvas播放时间（强制刷新）
            this.$emit('timeupdate', newTime, true)
        },
        formatTime(seconds) {
