<template>
  <transition name="fade">
    <div
      ...
      <!-- 弹窗内容 -->
      <div
        class="relative bg-white rounded-xl shadow-2xl w-80 mx-4 overflow-hidden"
        @click.stop>
        <!-- 标题栏 -->
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-lg bg-orange-100 flex items-center justify-center">
              <MusicNoteDotIcon class="w-4 h-4 text-orange-600" />
            </div>
            <h2 class="text-base font-semibold text-gray-800 leading-8 m-0">动态音符分析</h2>
          </div>
          <button
            @click="$emit('close')"
            class="w-8 h-8 rounded-full hover:bg-gray-100 transition-colors flex items-center justify-center">
            <CloseIcon class="w-4 h-4 text-gray-400" />
          </button>
        </div>
